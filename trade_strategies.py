...
#跌买涨卖相关参数
FALL_THRESHOLD = -0.01  # 单日跌幅阈值（-3%）
RISE_THRESHOLD = 0.03  # 卖出上涨阈值（4%）
...
# 可视化函数
def generate_interactive_html(df: pd.DataFrame, strategies_results: Dict[str, Dict[str, Any]],
                              output_path: str, start_date: str, end_date: str, stock_code: str, stock_name: str):
    fig = go.Figure()
    ...
    # 颜色和标记配置
    strategy_styles = {
        '跌买涨卖': {'buy_color': 'red', 'sell_color': 'green', 'line_color': 'blue',
                         'buy_symbol': 'triangle-up', 'sell_symbol': 'triangle-down'},
        '动态补仓': {'buy_color': 'purple', 'sell_color': 'orange', 'line_color': 'gray',
    ...
    for strategy_name, results in strategies_results.items():
        style = strategy_styles.get(strategy_name, {'buy_color': 'black', 'sell_color': 'black',
                                                    'line_color': 'black', 'buy_symbol': 'circle',
                                                    'sell_symbol': 'circle'})
